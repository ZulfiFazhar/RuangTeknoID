...
function EditPost() {
  ...
  useEffect(() => {
    ...
    const getPostDetail = async () => {
      try {
        ...
        const hashtags = res.data.data.hashtags.map((hashtagId) =>
          parseInt(hashtagId)
        );
        setPost({
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await api.put(`post/update-post-and-hashtags/${postId}`, post, {
      headers: {
      ...
      {authStatus.authStatus ? (
        <div>
          ...
          <div className="">
            {hashtags.map((tag) => (
              <div key={tag.hashtagId} className=" ">
                <input
                  type="checkbox"
                  checked={post.hashtags.includes(tag.hashtagId)}
                  onChange={(e) => {
                    if (e.target.checked) {
                      setPost((lp) => ({
                        ...lp,
                        hashtags: [...lp.hashtags, tag.hashtagId],
                      }));
                    } else {
                      setPost((lp) => ({
                        ...lp,
                        hashtags: lp.hashtags.filter(
                          (ht) => ht !== tag.hashtagId
                        ),
                      }));
                    }
          ...
          <button
            ...
            className="m-2 px-2 py-1 bg-red-400 rounded-md"
          >
            Delete Post
          </button>
        </div>
